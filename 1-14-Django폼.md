...
그런데 이렇게 되면, 관리자말고도 웹사이트를 접속한 일반 사용자가 마음대로 포스트를 수정하고 추가할 수 있을 것이다.  
여기서 생길수 있는 문제점을 막기위해서, 관리자만 수정이나 추가 페이지로 이동하게 변경하자.

방법은 간단하다. 폼 페이지로 넘어가는 a태그를 관리자가 아닌 일반 사용자는 볼 수 없도록 템플릿 태그로 감싸주면 된다.  
```html
{% if user.is_authenticated %}

{% endif %}
```
바로, user.is_authenticated 를 조건문으로 처리해주는 것인데, 이 .is_authenticated 는, 현재 웹페이지를 방문한 유저가 이 웹사이트에 로그인이 됬을때만 안에있는 html 코드를 보여주고,  
그렇지 않으면 이 템플릿태그안에 있는 html 코드는 무시되는 것이다.

이렇게 폼을 수정, 추가하는 링크를 관리자만 볼수 있게 해주었지만, 사용자가 폼을 수정, 추가하는 페이지의 링크를 안다면, 위의 방법으로는 막을 수 없을 것이다.  

이에 대해선, 폼을 수정, 추가하는 view함수에
```py3
@Login_required
```
라는 데코레이터(장식자)를 붙여서 로그인 된 상태인지 보장받을 수 있게 설정할 수 있다. (이부분은 튜토리얼 내용이 아니니 패스)

# 진짜 끝!

이제 튜토리얼의 모든 내용이 끝났다. 물론 이외에도 pythonanywhere로 배포하는 과정도 있지만, 배포에는 다양한 과정이 있고, 로컬로도 서버는 테스트해보기에 충분하기에 생략한다.

이제 이 다음단계로 이제 장고의 심화 부분에 대해서 알아보도록 하자.